...
from modules.io import ensure_dir, load_tokens_npz, save_image_rgb
from modules.pca import SubspaceModel, apply_percentile_bounds, project_tokens, upsample_components
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", type=str, default=None, help="仅运行指定单模型，等价于 --models <key>")
    parser.add_argument("model_keys", nargs="*", help="Positional shorthand for model keys (同 --models).")
...
def main() -> None:
    ...
    for spec in specs:
        ...
        if not subspace_path.exists() or not percentiles_path.exists():
            raise FileNotFoundError(f"Missing PCA artefacts for '{spec.key}'. Run knife_roi_pipeline first.")
