#!/usr/bin/env python3
"""Compute cosine similarity overlays between the knife anchor patch and other scenes."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", type=str, default=None, help="仅运行一个模型，等价于 --models <key>")
...
def main() -> None:
    ...
    for spec in specs:
        ...
        if not anchor_path.exists():
            raise FileNotFoundError(f"Missing anchor for '{spec.key}'. Run knife_patch_similarity first.")
        ...
        for name in ("kitchen", "office"):
            ...
            ensure_dir(sim_dir)
            npy_path = sim_dir / f"{name}_cosine_heatmap.npy"
            np.save(npy_path, sim_hw.astype(np.float32))
