...
# Minimal sequential runner for single_layer experiments (dinov2, sd, siglip)
# Usage: bash run_single_layer_extra.sh
...
declare -A BASE_CFG=(
  [dinov2]="$ROOT_DIR/configs/dinov2.yaml"
  [sd]="$ROOT_DIR/configs/sd21.yaml"
  [siglip]="$ROOT_DIR/configs/siglip.yaml"
)

MODELS=(sd siglip)

echo "Starting single_layer (dinov2, sd, siglip) sequential runs..."
...
for MODEL in "${MODELS[@]}"; do
  ...
  # Preferred explicit order per model
  ORDER=()
  if [[ "$MODEL" == "sd" ]]; then
    ORDER=(
      
      up2_normal
      up3_both up3_depth up3_normal
      up0_both up0_depth up0_normal
    )
  else
    ORDER=(
...
echo "All single_layer extra runs completed."
