...
借鉴 `vis_flux_cross_attention_map.py` 的做法，对 FLUX.1 Kontext pipeline
进行 cross-attention 捕获与可视化。运行过程中会：
  - 替换 19 个双流 Transformer block 的 FluxAttention Processor；
  - 记录图像查询对文本键的 softmax 概率；
  - 输出 per-token 以及 per-layer 的热点图；
  - 保存 attention 数组与 tokenizer 结果，用于后续分析。
...
if _torch_version_lt_25(torch.__version__):
    ...
    def _sdpa_wrapper(*args, **kwargs):
        kwargs.pop("enable_gqa", None)  # 老版本不支持该参数
...
def tokens_to_grid(values: np.ndarray, coords: np.ndarray) -> np.ndarray:
    """
    将 1D token 序列映射回二维栅格。
    coords: shape (B, N, 3) 或 (N, 3)，最后两个通道给出行列索引。
    约定第一维 == 0 表示图像主 token，其他值（如 1）为额外条件 token。
    """
    if coords.ndim == 3:
        coords = coords[0]
    if coords.shape[-1] != 3:
...
class FluxKontextAttnRecorderProcessor:
    """
    重写官方 FluxAttnProcessor，实现：
      - 捕获图像查询对文本键的注意力概率；
      - 保持返回签名与官方实现一致，确保 pipeline 行为不变。
    """

    _attention_backend = None  # 与官方保持一致
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="自动选择与输入图像尺寸最接近的 Kontext 分辨率（仅在未显式设置 height/width 时生效）",
    )
